import { PostsRepository } from '../repositories/postsRepository.mjs';

export class PostsService {
  // สร้างโพสต์ใหม่
  static async createPost(postData, imageUrl) {
    try {
  ...
  static async getAllPosts(queryParams) {
    try {
      ...
      const { data: posts, count: totalPosts, error, truePage, truelimit } = await PostsRepository.getAllPosts({
        page,
        limit,
        keyword,
        category,
  ...
  // อัปเดตโพสต์
  static async updatePost(postId, updateData, imageUrl) {
    try {
      const date = new Date();
      const { title, category_id, description, content, status_id } = updateData;

      // ตรวจสอบว่าโพสต์มีอยู่หรือไม่
      const { data: existingPost, error: checkError } = await PostsRepository.checkPostExists(postId);
  ...
  static async deletePost(postId) {
    try {
      ...
      if (error) {
        throw new Error("Server could not delete post because database connection");
      }
  ...
  static async getComments(postId, queryParams) {
    try {
      ...
      const { data: comments, count: totalComments, error, truePage, truelimit } = await PostsRepository.getComments(postId, {
        page,
        limit
      });
  ...
  // จัดการ like
  static async handleLikes(userId, postId) {
    try {
      const { data: existing, error: checkError } = await PostsRepository.checkLike(userId, postId);
      ...
      if (existing) {
        // ถ้ามี like อยู่แล้ว ให้ลบ
        await PostsRepository.removeLike(existing.id);
        await PostsRepository.updateLikesCount(postId, Math.max(post.likes_count - 1, 0));
