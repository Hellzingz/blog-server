...
export async function getAllPosts(queryParams) {
  try {
    // Business logic: Process query parameters
...
export async function updatePost(postId, updateData, imageUrl) {
  try {
    ...
    // Call Repository: Check if post exists
...
export async function createComment(postId, userId, commentText) {
  try {
    // Business logic: Prepare comment data
    const commentData = {
    ...
    // Business logic: Create notification for comment
    try {
      ...
      if (!postError && post && post.user_id !== userId) {
        // Get user data for notification message
        ...
        if (!userError && user) {
          await NotificationService.createNotification({
            ...
            actor_id: userId, // Commenter
          });
        }
      }
    } catch (notificationError) {
      console.error("Failed to create comment notification:", notificationError);
      // Don't throw error to avoid affecting comment creation
    }
...
export async function handleLikes(userId, postId) {
  try {
    // Call Repository: Check existing like
    ...
    // Business logic: Handle like/unlike
    if (existing) {
      ...
      // Business logic: Create notification for like (only when liking)
      try {
        ...
        // Don't throw error to avoid affecting like functionality
      }
...
export async function getPostTitles(status) {
  try {
    // Business logic: Process status parameter
